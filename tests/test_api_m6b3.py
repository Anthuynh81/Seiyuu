...
def test_unattributed_quote_surfaces_in_review_views(client, epub_bytes) -> None:
    """A quote the model failed to attribute (speaker None, quoted text, degraded
    confidence) must reach both review surfaces instead of hiding as narration."""
    book_id = _ingested(client, epub_bytes)
    report = AttributionReport(
        book_id=book_id,
        provider_id="local",
    ...
    assert flags == {
        ...
        ("ch001_b0002", 0): True,  # the quote nobody attributed
    }

    low = client.get(
        f"/api/books/{book_id}/chapters/1/segments", params={"low_confidence": True}
    ).json()["segments"]
    assert [(r["block_id"], r["speaker"], r["unattributed_quote"]) for r in low] == [
        ("ch001_b0002", None, True)
    ]
